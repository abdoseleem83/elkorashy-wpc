...
// ⚠️ الفاصلة: في الكيبورد العربي هي العلامة العشرية (1,5 = واحد ونص)، لكن لو
// المستخدم كتب فاصلة آلاف مع نقطة (1,234.5) كانت بتطلّع 1.2345 — طول عود
// غلط بالكامل. والفاصلة العربية ٬ (آلاف) كانت بتتقري عشرية.
const فواصل = await pg.evaluate(()=>{
  const t = v => ({ خرج: decIn_(v), رقم: Number(decIn_(v)) });
  return { عشرية_عربية: t('٢٢٠٫٥'), فاصلة_عادية: t('1,5'), آلاف_مع_نقطة: t('1,234.5'),
           آلاف_عربية: t('١٬٢٣٤'), مسافات: t(' 2 2 0 . 5 ') };
});
check('الفاصلة العربية ٫ عشرية', فواصل.عشرية_عربية.رقم === 220.5, JSON.stringify(فواصل.عشرية_عربية));
check('والفاصلة العادية عشرية لما تبقى لوحدها', فواصل.فاصلة_عادية.رقم === 1.5, JSON.stringify(فواصل.فاصلة_عادية));
check('وفاصلة الآلاف مع النقطة مابتبوّظش الرقم',
  فواصل.آلاف_مع_نقطة.رقم === 1234.5, JSON.stringify(فواصل.آلاف_مع_نقطة));
check('وفاصلة الآلاف العربية ٬ بتتشال', فواصل.آلاف_عربية.رقم === 1234, JSON.stringify(فواصل.آلاف_عربية));
check('والمسافات جوه الرقم بتتشال', فواصل.مسافات.رقم === 220.5, JSON.stringify(فواصل.مسافات));
