...
// ═══ مقاسات غير استاندر: سطر واحد بعدد الأعواد الإجمالي ═══
// ⚠️ السلوك ده اتغيّر بطلب من المصنع: كان بيتقسّم لسطرين (قوائم لوحدها وعوارض
// لوحدها)، وبقى سطر واحد بإجمالي الأعواد والأطوال مكتوبة في وصف السطر.
// الاختبار هنا بيحرس الحاجتين: العدد الإجمالي صح، والأطوال ما ضاعتش من الوصف.
const r2 = await add({ qty:3, jambCm:200, headerCm:120, nonStd:true });
check('٣ أطقم غير استاندر = سطر واحد', r2.سطور.length===1,
  JSON.stringify(r2.سطور.map(x=>x.عنوان)));
check('بإجمالي ٩ أعواد (٦ قوائم + ٣ عوارض)', r2.سطور[0].كمية===9, String(r2.سطور[0].كمية));
check('والقوائم بطولها وعددها مكتوبين في السطر',
  /قائم 200 سم\s*×\s*6/.test(r2.سطور[0].عنوان), r2.سطور[0].عنوان);
check('والعوارض بطولها وعددها كمان — ما ضاعتش',
  /عارضة حلق علوية 120 سم\s*×\s*3/.test(r2.سطور[0].عنوان), r2.سطور[0].عنوان);

// الفلوس: سطر واحد بسعر وحدة واحد لأعواد أطوالها مختلفة — لازم الإجمالي يفضل صح
const r2p = await pg.evaluate(()=>{
  const f = FRAMES[0];
  return { صح: Math.round((6*rodPrice(f.price,200) + 3*rodPrice(f.price,120))*100)/100 };
});
check('إجمالي السطر المدموج = مجموع الأعواد بأطوالها (فرق التقريب أقل من قرش)',
  Math.abs(r2.إجمالي - r2p.صح) < 0.05, `${r2.إجمالي} مقابل ${r2p.صح}`);
