...
// ⚠️ الارتفاع الاستاندر (٢١٥) بيتكتب صراحة في الطلبات الراجعة من السيرفر
// ومن نص الطلب. لو اتحسب «ارتفاع مخصوص» يبقى كل باب عادي عليه ٣٠٠ ج قص
// من غير سبب. الحد ده مكانش عليه أي اختبار.
const ارتفاعات = await pg.evaluate(()=>{
  window.toast=()=>{}; window.render=()=>{};
  const جرّب = (h)=>{
    state.cart = [{ kind:'door', code:'A01', title:'باب A01', sizeTxt:'70 سم', sizeEn:'70 cm',
                    w:70, qty:2, unitPrice:priceForWidth(70,false), doorHeight:h, hafr:false, wood:false }];
    saveCart_();
    return state.cart.filter(it=>String(it.code)==='CUT').length;
  };
  const r = { استاندر: جرّب(DOOR_STD_HEIGHT), فاضي: جرّب(0), أقصر: جرّب(205), أطول: جرّب(240),
              نص_الاستاندر: DOOR_STD_HEIGHT };
  state.cart = []; saveCart_();
  return r;
});
check('الارتفاع الاستاندر (٢١٥) مالوش بند قص',
  ارتفاعات.استاندر === 0, 'ارتفاع '+ارتفاعات.نص_الاستاندر+' → '+ارتفاعات.استاندر+' بند');
check('والارتفاع الفاضي كمان', ارتفاعات.فاضي === 0, String(ارتفاعات.فاضي));
check('لكن الأقصر من الاستاندر عليه قص', ارتفاعات.أقصر === 1, String(ارتفاعات.أقصر));
check('والأطول كمان', ارتفاعات.أطول === 1, String(ارتفاعات.أطول));
